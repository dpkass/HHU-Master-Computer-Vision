...
def display_matches(image1, image2, filtered_matched_kp, k=100):
    ...
    idxs = np.random.choice(len(filtered_matched_kp), size=k, replace=False)
    points1 = filtered_matched_kp[idxs, 0]
    points2 = filtered_matched_kp[idxs, 1]
...
def pad_and_stack(aligned_images):
    ...
    """
    max_val = max(i.max() for i in aligned_images)
